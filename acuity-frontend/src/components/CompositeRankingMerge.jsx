...
const CompositeRankingMerge = () => {
  ...
  const [step, setStep] = useState(0); // 0: initial, 1: text score, 2: dist score, 3: merge
  ...
  return (
    <div className="card mt-6">
      <h3 style={{ fontSize: '1.25rem', fontWeight: 800, marginBottom: '8px' }}>
        Algorithm Integration: Weighted Merge
      </h3>
      ...
      {/* Flow Diagram */}
      <div style={{ padding: '24px', background: 'var(--bg-elevated)', borderRadius: 'var(--radius-card)', marginBottom: '24px', position: 'relative' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px', position: 'relative' }}>

          {/* Cosine Node */}
          <div style={{
            width: '40%', padding: '16px', background: 'rgba(59, 130, 246, 0.1)',
            border: step >= 1 ? '2px solid #3b82f6' : '2px dashed var(--border)',
            borderRadius: 'var(--radius-card)', textAlign: 'center',
            transition: isReducedMotion ? 'none' : 'border 0.3s ease, box-shadow 0.3s ease',
            ...
            <div style={{ marginTop: '8px', fontWeight: 700, fontSize: '1.2rem', opacity: step >= 1 ? 1 : 0.3 }}>
          ...
          {/* Haversine Node */}
          <div style={{
            width: '40%', padding: '16px', background: 'rgba(236, 72, 153, 0.1)',
            ...
            <div style={{ marginTop: '8px', fontWeight: 700, fontSize: '1.2rem', opacity: step >= 2 ? 1 : 0.3 }}>
        ...
        <div style={{ position: 'absolute', top: '100px', left: '0', right: '0', height: '60px', pointerEvents: 'none' }}>
           <svg width="100%" height="100%" style={{ overflow: 'visible' }}>
             {/* Left Arrow Path */}
             <path d="M 20% 0 C 20% 40, 50% 40, 50% 60" fill="none" stroke={step >= 3 ? '#3b82f6' : 'var(--border)'} strokeWidth="3" strokeDasharray={step >= 3 && !isReducedMotion ? "8,8" : "none"} strokeLinecap="round">
               {step >= 3 && !isReducedMotion && <animate attributeName="stroke-dashoffset" from="16" to="0" dur="0.5s" repeatCount="indefinite" />}
             </path>
             {/* Right Arrow Path */}
             <path d="M 80% 0 C 80% 40, 50% 40, 50% 60" fill="none" stroke={step >= 3 ? '#ec4899' : 'var(--border)'} strokeWidth="3" strokeDasharray={step >= 3 && !isReducedMotion ? "8,8" : "none"} strokeLinecap="round">
               {step >= 3 && !isReducedMotion && <animate attributeName="stroke-dashoffset" from="16" to="0" dur="0.5s" repeatCount="indefinite" />}
             </path>
           </svg>
        </div>
        ...
        {/* Composite Node */}
        <div style={{ 
          width: '50%', margin: '0 auto', padding: '16px', 
          background: step >= 3 ? 'linear-gradient(135deg, rgba(59, 130, 246, 0.1), rgba(236, 72, 153, 0.1))' : 'var(--bg-card)', 
          ...
          <code className="text-xs text-secondary" style={{ background: 'transparent' }}>(Text × α) + (Prox × �)</code>
        </div>
      </div>
      ...
      {/* Interactive Ranking Sample */}
      <div>
        <h4 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '12px' }}>Live Re-ranking Example</h4>
