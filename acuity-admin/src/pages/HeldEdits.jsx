...
function HeldEdits() {
  ...
  const getDiff = (proposed, currentData) => {
    ...
    for (const key in proposed) {
      if (['id', 'history', 'stats', 'flagCount', 'flagReasons', 'originalIndex', 'contact', 'contact_info', 'published_at', 'updated_at', 'created_at', 'pin', 'business_name', 'ownerId', 'edit_version', 'sensitive_fields'].includes(key)) continue;
